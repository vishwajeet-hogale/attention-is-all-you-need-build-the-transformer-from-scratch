...
def merge_heads_and_project_output(context, w_o, b_o):
    ...
    B, H, L, d_k = context.shape
    merged = context.transpose(1, 2).reshape(B, L, H * d_k)   # (B, L, d_model)
    if b_o is not None:
...
def apply_residual_add_and_norm(residual_input, sublayer_output, gamma, beta, eps=1e-5):
    ...
    return normalize_and_scale_with_gamma_beta(output, gamma, beta, eps)
...
def decoder_layer_feed_forward_sublayer(y, w1, b1, w2, b2, gamma, beta):
    ...
    ouput = position_wise_feed_forward_network(y, w1, b1, w2, b2)
    return apply_residual_add_and_norm(y, ouput, gamma, beta)
...
def apply_final_output_projection(decoder_output, output_projection_weight, output_projection_bias=None):
    ...
    out = decoder_output @ output_projection_weight.transpose(-1,-2)
    if output_projection_bias is not None:
        out = out + output_projection_bias
    return out
